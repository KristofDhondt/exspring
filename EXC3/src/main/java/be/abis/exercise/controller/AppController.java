...
public class AppController {
	...
	@PostMapping("/")
	public String welcome (Model model, @Valid Login loginUser, BindingResult bindingResult) {
		if (bindingResult.hasErrors()) {
			return "login";
	    }
		loggedInUser = trainingService.findPerson(loginUser.getEmail(),loginUser.getPassword());
		if (loggedInUser==null) {
			   bindingResult.reject("email", "Login failed, please try again");
			   return "login";
			}
	...
	public String addPerson(@Valid Person person, BindingResult bindingResult) {	
		try {
			...
			bindingResult.reject("PersonId", "Person already exists");
			return "addPerson";
		}	
		return "redirect:/personadmin";
	}
	...
	public String findPersonById(@Valid Person person, BindingResult bindingResult) {
		Person personById = trainingService.findPerson(person.getPersonId());
		if (personById==null) {
			bindingResult.reject("personId", "No person found");
			return "searchperson";
		}
